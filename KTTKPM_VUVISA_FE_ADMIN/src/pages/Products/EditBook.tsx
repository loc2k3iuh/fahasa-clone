...
const EditBook: React.FC = () => {
  ...
  const loadCategoryOptions = async (inputValue: string) => {
    ...
    // Chỉ lấy SACH_NUOC_NGOAI hoặc SACH_TRONG_NUOC
    return arr
      .filter(
        (c) =>
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const toastId = toast.loading("Loading book data...");
      try {
        const productRes = await apiClient.get(`/products/books/${id}`);
        ...
        setPublisherId({ id: product.publisher.id, name: product.publisher.publisherName });
        setIsbn(product.isbn);
        setPublisherDate(product.publisherDate);
        setCurrentAuthors(product.authors || []);
        setImageProducts(product.imageProducts || []);

        toast.success("Loaded book data!", { id: toastId });
      } catch (error) {
        toast.error("Failed to load book data.", { id: toastId });
      }
    };
    fetchData();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Uploaded image products:", uploadedImageProducts); // Log để kiểm tra dữ liệu ảnh đã upload
      ...
      const updatedProduct = {
        ...
        category: { id: categoryId.id }, // Gửi category ID
        supplier: { id: supplierId.id }, // Gửi supplier ID
        publisher: { id: publisherId.id }, // Gửi publisher ID
        imageProducts: uploadedImageProducts,
        isbn,
        publisherDate,
        // publisher: { id: publisherId },
        authors: currentAuthors.map((author) => ({ id: author.id })), // Chỉ gửi ID của tác giả
      };

      console.log(updatedProduct)

      // Gửi dữ liệu cập nhật
      await apiClient.put(`/products/books/${id}`, updatedProduct, {
        headers: {
  ...
  // Author multi select
  const handleAuthorsChange = (options: any) => {
    setCurrentAuthors(
    ...
    <main className="bg-gray-900">
      <div className="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6 mb-15">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-xl font-semibold text-white/90">Edit Product</h2>
                      <nav>
                  ...
                  <video
                    src={
                        videoFile instanceof File
                          ...
                          : (videoFile as any).url // video cũ từ API
                      }
                    controls
